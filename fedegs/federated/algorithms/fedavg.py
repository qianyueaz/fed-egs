...
class FedAvgServer(BaseFederatedServer):
    ...
    def train(self, test_dataset: Dataset) -> List[RoundMetrics]:
        ...
        for round_idx in range(1, self.config.federated.rounds + 1):
            ...
            LOGGER.info(
                "fedavg round %d | loss=%.4f | personalized_acc=%.4f | weighted_acc=%.4f | hard_recall=%.4f | savings=%.4f%s",
                round_idx,
                avg_loss,
                macro["accuracy"],
                aggregate["accuracy"],
            ...
            self._log_auxiliary_accuracy_metrics("fedavg", round_idx, expert_accuracy=None, general_accuracy=macro["accuracy"])
